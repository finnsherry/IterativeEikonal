...
import h5py
import taichi as ti
from eikivp.W2.plus.interpolate import vectorfield_trilinear_interpolate_LI
...
def import_γ_path(params, folder):
    ...
    if cost_domain == "M2":
        ...
        geodesic_filename = f"{folder}\\W2_p_ss_s={[s for s in σ_s_list]}_s_o={σ_o}_s_s_e={σ_s_ext}_s_o_e={σ_o_ext}_l={λ}_p={p}_x={ξ}_t={target_point}.hdf5"
        ...
        γ = params["γ"]
        ε = params["ε"]
        geodesic_filename = f"{folder}\\W2_p_ss={[s for s in scales]}_a={α}_g={γ}_e={ε}_l={λ}_p={p}_x={ξ}_t={target_point}.hdf5"
        with h5py.File(geodesic_filename, "r") as geodesic_file:
            if "source_point" in params:
                source_point = params["source_point"]
                assert (
...
def geodesic_back_tracking(
    ...
    while (distance >= tol) and (n < n_max - 1):
        point = geodesic_back_tracking_step(
            grad_W, point, αs, φs, ξ, cost, α_min, β_min, φ_min, dα, dβ, dφ, dt
        )

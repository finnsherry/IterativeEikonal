...
connecting two points in W2. The primary method is:
...
import h5py
import taichi as ti
from eikivp.W2.Riemannian.interpolate import vectorfield_trilinear_interpolate_LI
...
def import_γ_path(params, folder):
    ...
    if cost_domain == "M2":
        ...
        geodesic_filename = f"{folder}\\W2_R_ss_s={[s for s in σ_s_list]}_s_o={σ_o}_s_s_e={σ_s_ext}_s_o_e={σ_o_ext}_l={λ}_p={p}_G={[g for g in G]}_t={target_point}.hdf5"
        ...
        γ = params["γ"]
        ε = params["ε"]
        geodesic_filename = f"{folder}\\W2_R_ss={[s for s in scales]}_a={α}_g={γ}_e={ε}_l={λ}_p={p}_G={[g for g in G]}_t={target_point}.hdf5"
        with h5py.File(geodesic_filename, "r") as geodesic_file:
            if "source_point" in params:
                source_point = params["source_point"]
                assert (
...
def geodesic_back_tracking(
    ...
    source_point = coordinate_array_to_real(
        *source_point, α_min, β_min, φ_min, dα, dβ, dφ
    )
    target_point = coordinate_array_to_real(
        *target_point, α_min, β_min, φ_min, dα, dβ, dφ
    )
    ...
    while (distance >= tol) and (n < n_max - 1):
        point = geodesic_back_tracking_step(
            grad_W, point, αs, φs, G, cost, α_min, β_min, φ_min, dα, dβ, dφ, dt
        )
...
def geodesic_back_tracking_multi_source(
    ...
    source_points_np = np.array(
        tuple(
            coordinate_array_to_real(*p, α_min, β_min, φ_min, dα, dβ, dφ)
    ...
    while (min_distance >= tol) and (n < n_max - 1):
        ...
        min_distance = distance_in_pixels_multi_source(
            point, source_points, distances, dα, dβ, dφ
        )
        γ[n] = point
        n += 1
    γ_len = n

...
from eikivp.W2.Riemannian.metric import invert_metric
...
def eikonal_solver(
    ...
    ε = dε / np.sqrt(
        (G_inv[0] + G_inv[1] + G_inv[2]) / min(dα, dβ, dφ) ** 2
    )  # * cost_np.min()
...
def distance_gradient_field(
    ...
    for I in ti.grouped(B1_W):
        grad_W[I] = (
            ti.Vector([G_inv[0] * B1_W[I], G_inv[1] * B2_W[I], G_inv[2] * B3_W[I]])
            / cost[I] ** 2
        )
...
def step_W_uniform(
    ...
    for I in ti.grouped(W):
        ...
        dW_dt[I] = 1 - ti.math.sqrt(
            G_inv[0] * B1_W[I] ** 2 + G_inv[1] * B2_W[I] ** 2 + G_inv[2] * B3_W[I] ** 2
        )
        W[I] += dW_dt[I] * ε
...
def distance_gradient_field_uniform(
    ...
    for I in ti.grouped(B1_W):
        grad_W[I] = ti.Vector(
            [G_inv[0] * B1_W[I], G_inv[1] * B2_W[I], G_inv[2] * B3_W[I]]
        )

"""
    ...
    Provides tools to compute vesselness scores on M2. The available methods
    are:
      ...
      projection.
    Additionally, we have code for regularising functions on M2, namely:
    ...
    We use that the spatially isotropic diffusion equation on M2 can be
...
import h5py
import taichi as ti
from eikivp.M2.utils import sanitize_index
...
def import_vesselness(params, folder):
    ...
    with h5py.File(vesselness_filename, "r") as vesselness_file:
        assert (
            ...
            σ_o == vesselness_file.attrs["σ_o"] and
            σ_s_ext == vesselness_file.attrs["σ_s_ext"] and
            σ_o_ext == vesselness_file.attrs["σ_o_ext"] and
...
def export_vesselness(V, params, folder):
    """
    Export the vesselness to hdf5 with attributes `params` stored as metadata.
    """
    σ_s_list = params["σ_s_list"]
    σ_o = params["σ_o"]
    σ_s_ext = params["σ_s_ext"]
    ...
    with h5py.File(vesselness_filename, "w") as vesselness_file:
        ...
        vesselness_file.attrs["σ_o"] = σ_o
        vesselness_file.attrs["σ_s_ext"] = σ_s_ext
        vesselness_file.attrs["σ_o_ext"] = σ_o_ext

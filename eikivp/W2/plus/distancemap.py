...
Provides methods to compute the distance map on W2 with respect to various
metrics, by solving the Eikonal PDE using the iterative Initial Value
Problem (IVP) technique first described by Bekkers et al.[2] and generalised
in [1]. The primary methods are:
  ...
  to the B3-direction (it corresponds to β used by Bekkers et al.[1]);
...
def import_W(params, folder):
    ...
    if cost_domain == "M2":
        ...
        if "source_point" in params:
            ...
            distance_filename = f"{folder}\\W2_p_ss_s={[s for s in σ_s_list]}_s_o={σ_o}_s_s_ext={σ_s_ext}_s_o_ext={σ_o_ext}_l={λ}_p={p}_x={ξ}_s={source_point}.hdf5"
            ...
            distance_filename = f"{folder}\\W2_p_ss_s={[s for s in σ_s_list]}_s_o={σ_o}_s_s_ext={σ_s_ext}_s_o_ext={σ_o_ext}_l={λ}_p={p}_x={ξ}.hdf5"
            with h5py.File(distance_filename, "r") as distance_file:
                assert (
                    np.all(σ_s_list == distance_file.attrs["σ_s_list"])
                    and σ_o == distance_file.attrs["σ_o"]
        ...
        if "source_point" in params:
            source_point = params["source_point"]
            distance_filename = f"{folder}\\W2_p_ss={[s for s in scales]}_a={α}_g={γ}_e={ε}_l={λ}_p={p}_x={ξ}_s={source_point}.hdf5"
            with h5py.File(distance_filename, "r") as distance_file:
                assert (
            ...
            source_points = params["source_points"]
            distance_filename = f"{folder}\\W2_p_ss={[s for s in scales]}_a={α}_g={γ}_e={ε}_l={λ}_p={p}_x={ξ}.hdf5"
            with h5py.File(distance_filename, "r") as distance_file:
                assert (
                    np.all(scales == distance_file.attrs["scales"])
                    and α == distance_file.attrs["α"]
...
def eikonal_solver_multi_source(
    ...
    Solve the Eikonal PDE on W2 equipped with a datadriven left invariant
    Finsler function defined by `ξ` and `cost_np`, with source at
    `source_points`, using the iterative method first described by Bekkers et
    al.[2] and generalised in [1].
...
def step_W(
    ...
    for I in ti.grouped(W):
        ...
        dW_dt[I] = (
            ...
            - (
                ti.math.sqrt(
                    soft_plus(B1_W[I], plus_softness) ** 2 / ξ**2 + B3_W[I] ** 2
                )
                / cost[I]
            )
        ) * cost[I]
...
def distance_gradient_field(
    ...
    for I in ti.grouped(B1_W):
        grad_W[I] = (
            ti.Vector([soft_plus(B1_W[I], plus_softness) / ξ**2, 0.0, B3_W[I]])
            / cost[I] ** 2
        )
...
def distance_gradient_field_uniform(
    ...
    upwind_B3(W, dφ, B3_forward, B3_backward, B3_W)
    for I in ti.grouped(B1_W):
        grad_W[I] = ti.Vector([soft_plus(B1_W[I], plus_softness) / ξ**2, 0.0, B3_W[I]])

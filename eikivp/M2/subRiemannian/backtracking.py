...
import h5py
import taichi as ti
from eikivp.M2.subRiemannian.interpolate import vectorfield_trilinear_interpolate_LI
...
def import_γ_path(params, folder):
    ...
    if cost_domain == "M2":
        ...
        geodesic_filename = f"{folder}\\M2_sR_ss_s={[s for s in σ_s_list]}_s_o={σ_o}_s_s_e={σ_s_ext}_s_o_e={σ_o_ext}_l={λ}_p={p}_x={ξ}_t={target_point}.hdf5"
        ...
        γ = params["γ"]
        ε = params["ε"]
        geodesic_filename = f"{folder}\\M2_sR_ss={[s for s in scales]}_a={α}_g={γ}_e={ε}_l={λ}_p={p}_x={ξ}_t={target_point}.hdf5"
        with h5py.File(geodesic_filename, "r") as geodesic_file:
            if "source_point" in params:
                source_point = params["source_point"]
                assert (

...
from eikivp.SO3.plus.distancemap import (
    DistanceSO3Plus,
    DistanceMultiSourceSO3Plus
)
...
class GeodesicMultiSourceSO3Plus():
    """
    Compute the geodesic of a plus controller distance map on SO(3).
    ...
    def __init__(self, W: DistanceMultiSourceSO3Plus, target_point=None, dt=1.):
        # Vesselness attributes
        self.σ_s_list = W.σ_s_list
        self.σ_o = W.σ_o
        self.σ_s_ext = W.σ_s_ext
    ...
    def compute_γ_path(self, W: DistanceMultiSourceSO3Plus, C: CostSO3, α_min, β_min, φ_min, dα, dβ, dφ, αs_np, φs_np,
                       n_max=2000):
    ...
    def import_γ_path(self, folder):
        ...
        `target_point`.
        """
        geodesic_filename = f"{folder}\\SO3_p_ss_s={[s for s in self.σ_s_list]}_s_o={self.σ_o}_s_s_e={self.σ_s_ext}_s_o_e={self.σ_o_ext}_l={self.λ}_p={self.p}_x={self.ξ}_t={self.target_point}.hdf5"
        with h5py.File(geodesic_filename, "r") as geodesic_file:
            assert (
                np.all(self.σ_s_list == geodesic_filename.attrs["σ_s_list"]) and
                self.σ_o == geodesic_filename.attrs["σ_o"] and

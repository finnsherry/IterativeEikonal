...
from eikivp.SE2.Riemannian.distancemap import (
    DistanceSE2Riemannian,
    DistanceMultiSourceSE2Riemannian
)
...
class GeodesicMultiSourceSE2Riemannian():
    """
    Compute the geodesic of a Riemannian distance map on SE(2).
    ...
    def __init__(self, W: DistanceMultiSourceSE2Riemannian, target_point=None, dt=1.):
        # Vesselness attributes
        self.σ_s_list = W.σ_s_list
        self.σ_o = W.σ_o
        self.σ_s_ext = W.σ_s_ext
    ...
    def compute_γ_path(self, W: DistanceMultiSourceSE2Riemannian, C: CostSE2, x_min, y_min, θ_min, dxy, dθ, θs_np, n_max=2000):
        self.γ_path = geodesic_back_tracking_multi_source(W.grad_W, self.source_points, self.target_point, C.C, x_min,
                                                          y_min, θ_min, dxy, dθ, θs_np, self.G, dt=self.dt, n_max=n_max)
    ...
    def import_γ_path(self, folder):
        ...
        `target_point`.
        """
        geodesic_filename = f"{folder}\\SE2_R_ss_s={[s for s in self.σ_s_list]}_s_o={self.σ_o}_s_s_e={self.σ_s_ext}_s_o_e={self.σ_o_ext}_l={self.λ}_p={self.p}_G={[g for g in self.G]}_t={self.target_point}.hdf5"
        with h5py.File(geodesic_filename, "r") as geodesic_file:
            assert (
                np.all(self.σ_s_list == geodesic_filename.attrs["σ_s_list"]) and
                self.σ_o == geodesic_filename.attrs["σ_o"] and

...
def import_W(params, folder):
    ...
    if cost_domain == "M2":
        ...
        if "source_point" in params:
            ...
            distance_filename = f"{folder}\\M2_sR_ss_s={[s for s in σ_s_list]}_s_o={σ_o}_s_s_ext={σ_s_ext}_s_o_ext={σ_o_ext}_l={λ}_p={p}_x={ξ}_s={source_point}.hdf5"
            ...
            distance_filename = f"{folder}\\M2_sR_ss_s={[s for s in σ_s_list]}_s_o={σ_o}_s_s_ext={σ_s_ext}_s_o_ext={σ_o_ext}_l={λ}_p={p}_x={ξ}.hdf5"
            with h5py.File(distance_filename, "r") as distance_file:
                assert (
                    np.all(σ_s_list == distance_file.attrs["σ_s_list"])
                    and σ_o == distance_file.attrs["σ_o"]
        ...
        if "source_point" in params:
            source_point = params["source_point"]
            distance_filename = f"{folder}\\M2_sR_ss={[s for s in scales]}_a={α}_g={γ}_e={ε}_l={λ}_p={p}_x={ξ}_s={source_point}.hdf5"
            with h5py.File(distance_filename, "r") as distance_file:
                assert (
            ...
            source_points = params["source_points"]
            distance_filename = f"{folder}\\M2_sR_ss={[s for s in scales]}_a={α}_g={γ}_e={ε}_l={λ}_p={p}_x={ξ}.hdf5"
            with h5py.File(distance_filename, "r") as distance_file:
                assert (
                    np.all(scales == distance_file.attrs["scales"])
                    and α == distance_file.attrs["α"]
...
def eikonal_solver(
    ...
    ε = dε / np.sqrt(1 / (ξ * dxy) ** 2 + 1 / dθ**2)  # * cost_np.min()
    print(f"ε = {ε}")
...
def step_W(
    ...
    for I in ti.grouped(W):
        ...
        dW_dt[I] = (
            1 - (ti.math.sqrt(A1_W[I] ** 2 / ξ**2 + A3_W[I] ** 2) / cost[I])
        ) * cost[I]
        W[I] += dW_dt[I] * ε
...
def distance_gradient_field(
    ...
    for I in ti.grouped(A1_W):
        grad_W[I] = ti.Vector([A1_W[I] / ξ**2, 0.0, A3_W[I]]) / cost[I] ** 2
...
def step_W_uniform(
    ...
    for I in ti.grouped(W):
        # It seems like TaiChi does not allow negative exponents.
        dW_dt[I] = 1 - ti.math.sqrt(A1_W[I] ** 2 / ξ**2 + A3_W[I] ** 2)
        W[I] += dW_dt[I] * ε
...
def distance_gradient_field_uniform(
    ...
    upwind_A3(W, dθ, A3_forward, A3_backward, A3_W)
    for I in ti.grouped(A1_W):
        grad_W[I] = ti.Vector([A1_W[I] / ξ**2, 0.0, A3_W[I]])
